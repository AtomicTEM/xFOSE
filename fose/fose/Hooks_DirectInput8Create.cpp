...
void Hook_DirectInput8Create_Init() {
	...
	UInt32 thunkAddress = 0x00D9402C;
#elif FALLOUT_VERSION == FALLOUT_VERSION_1_1_35
	UInt32 thunkAddress = 0x00D9902C;
	...
	UInt32 thunkAddress = 0x00D9B02C;
	...
	UInt32 thunkAddress = 0x00D9802C;
...
bool sDI_data::IsKeyPressed(UInt32 keycode)
{
	if (keycode < kMaxMacros && LastBytes[keycode])
		return true;

	return false;
}

bool sDI_data::IsKeyDisabled(UInt32 keycode)
{
	if (keycode < kMaxMacros && DisallowStates[keycode] == 0)
		return true;

	return false;
}

bool sDI_data::IsKeyHeld(UInt32 keycode)
{
	if (keycode < kMaxMacros && FakeStates[keycode] == 0x80)
		return true;

	return false;
}

bool sDI_data::IsKeyTapped(UInt32 keycode)
{
	if (keycode < kMaxMacros && TapStates[keycode] == 0x80)
		return true;

	return false;
}

void sDI_data::SetKeyDisableState(UInt32 keycode, bool bDisable)
{
	if (keycode < kMaxMacros)
		DisallowStates[keycode] = bDisable ? 0 : 0x80;
}

void sDI_data::SetKeyHeldState(UInt32 keycode, bool bHold)
{
	if (keycode < kMaxMacros)
		FakeStates[keycode] = bHold ? 0x80 : 0;
}

void sDI_data::TapKey(UInt32 keycode)
{
	if (keycode < kMaxMacros)
